...
# from utils.session_manager import session_exists, is_redirected_to_sso
# # ============================================================
# # utils/login.py
# #
...
def handle_login(els, email, password, url):
    ...
    if not _login.session_checked:
        ...
        if session_exists():
            if not is_redirected_to_sso(url):
                ...
                print("[LOGIN] Session stale → will re-login (handled by main)")

    # ============================================================
    # 🔹 Session expired (IMPORTANT FIX)
    # ============================================================
    if is_redirected_to_sso(url) and _login.session_checked:
        print("[LOGIN] Session expired → letting main handle re-login")
        return {"action": "wait", "seconds": 1}

    # ============================================================
    # 🔹 SSO fallback (rare case)
    # ============================================================
    is_sso = "microsoftonline.com" in url.lower()
    is_app = BASE_URL.replace("https://", "") in url.lower()
    ...
    if is_app and not is_sso:
        ...
        if dom_count == 0:
            _login._empty_dom_count += 1
            print(f"[LOGIN] DOM empty ({_login._empty_dom_count}/{_login.MAX_EMPTY_DOM}) — waiting")
    ...
    if yb:
        ...
        return {
            "action": "click",
            "selector": yb["selector"],
            "sso_yes": True,
            "soft_fail": True
        }

    # ============================================================
    # 🔹 Already logged in (no inputs)
    # ============================================================
    if not e and not pw and len(els.get("dom_raw") or []) > 0:
        print("[LOGIN] No credentials form — treating as logged in")
        _login.done = True
