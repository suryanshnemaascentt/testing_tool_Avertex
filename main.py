...
async def run(url, module_key, action_key, goal, test_mode=False):
    ...
    async with async_playwright() as p:
        ...
        # ── STEP 0b: Check session not expired after load ─────
        if is_redirected_to_sso(page.url):
            print("[MAIN] Session expired — deleting session.json")
            print("[MAIN] Please re-run — auto-login will trigger again.")
            delete_session()
            await browser.close()
            if reporter:
                reporter.start()
        ...
        for step_num in range(MAX_STEPS):
            ...
            if is_redirected_to_sso(current_url):
                print("[MAIN] Unexpected SSO redirect — session expired mid-run")
                delete_session()
                if reporter:
