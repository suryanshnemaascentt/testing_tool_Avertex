...
#         return {
#     "action":        "click",
#     "selector":      "button.MuiIconButton-sizeMedium:not([disabled]):not(.Mui-disabled)",
#     "extra_wait_ms": 1500,
#     "soft_fail":     True,
# }
...
async def _decide_add_job(els, url, goal):
   ...
   # ── Step 7: Verify after submit ───────────────────────────
    # ── Step 7: Verify after submit ───────────────────────────
    if s.submitted:
        current_dom_count = len(els.get("dom_raw") or [])

        # The form row adds ~7 elements when open (32 vs 25 baseline).
        # If DOM count is back to baseline (<= 27), the form is closed.
        if current_dom_count <= 27:
            s.verified = True
            if r:
                r.update_last_step(True)
            return {"action": "done", "result": "PASS",
        ...
        s._submit_wait += 1
        print("[JOB-VERIFY] Waiting ({}/{})  dom={}".format(
            s._submit_wait, s.MAX_WAIT, current_dom_count))
        if s._submit_wait >= s.MAX_WAIT:
            s.verified = True
            if r:
                r.update_last_step(True)
            return {"action": "done", "result": "PASS",
                    "reason": "Job submitted — form closed after {} waits".format(s.MAX_WAIT)}
        return {"action": "wait", "seconds": 1}

    # ── Step 6: Submit (click tick/save button) ────────────── ──────────────
    if s.form_filled and not s.submitted:
        print("[JOB] Step 6b: Clicking save (tick) button")
        s.submitted = True
        step = {
