...
# Strict delete success phrases only — avoids false positives
# from project names containing words like "updated" or "deleted"
    ...
    for el in dom:
        ...
        if tag in _BTN_TAGS and etype in _BTN_TYPES:
            if "next" in lv and not result["next_btn"]:
                ...
                result["edit_btn"] = el
            # FIX: detect delete button by MUI error class (reliable)
            # button:has-text('Delete') fails because the button contains
            # an SVG icon before the text — class name is always present
            elif (
                text == "delete" or
                label == "delete" or
                "muibutton-outlinederror" in cls or
                "muibutton-colorerror" in cls
            ):
...
async def decide_create(els, url):
    ...
    if s.submitted:
        ...
        # Method B: project name visible in DOM
...
async def decide_delete(els, url, goal):
    ...
    # The app uses window.confirm() — Playwright's dialog handler
    # in main.py auto-accepts it. So after delete click + 2s wait,
    # the dialog is already accepted. We just mark confirmed and move
    # to verification immediately.
    if s.delete_clicked:
    ...
    if s.search_typed:
        ...
        if db and db["selector"] not in s.interacted:
            ...
            # FIX: use reliable MUI class selector instead of db["selector"]
            # button:has-text('Delete') fails because the button HTML contains
            # an SVG icon before the text node. MuiButton-outlinedError class
            # is always present on delete buttons — confirmed by browser inspect.
